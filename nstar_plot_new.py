...
class nstar_plot(load_crust):
    ...
    def mag_field(self,ord):
        ...
        self.pb.arrow(0.5+0.2*cos(ang2),0.5+0.2*sin(ang2),
                      0.23*cos(ang2),0.23*sin(ang2),
                      'head_width=0.01,head_length=0.03',
                      color='cyan',zorder=ord)
        # Arrow for magnetic south
        self.pb.arrow(0.5+0.4*cos(ang2+self.pi),
    ...
    def cutaway(self,factor,cname,ord):
        ...
        self.pb.axes.fill_between(x,bot_y,top_y,facecolor=cname,lw=0,zorder=ord)
    ...
    def cutaway_axes(self,ord):
        ...
        plot.plot([0.5,0.41],[0.5,0.41],color='black',ls='-',lw=1.5,
                  zorder=ord)
        self.pb.axes.text(0.51,0.44,r'$R{\approx}10-13$ km',
    ...
    def cut_labels(self,ord):
        self.pb.axes.text(0.69,0.95,'Atmos.: H, He, C',fontsize=self.font_scale*0.7,
                     color=self.atmos_color,va='center',ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.69,0.90,'Outer Crust',fontsize=self.font_scale*0.7,
                     color=self.crust_color,va='center',ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.69,0.85,'(Z,N)+e',fontsize=self.font_scale*0.7,
                     color=self.crust_color,va='center',ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.69,0.80,'Inner crust',fontsize=self.font_scale*0.7,
                     color=self.crust_color,va='center',ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.69,0.75,'(Z,N)+e+n',fontsize=self.font_scale*0.7,
                     color=self.crust_color,va='center',ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.69,0.70,'Outer Core: n+p+e',fontsize=self.font_scale*0.7,
                     color=self.core_color,va='center',ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.69,0.65,'Inner Core: ?',fontsize=self.font_scale*0.7,
    ...
    def crust_box(self,ord):
        ...
        for i in range(0,60):
            ...
            self.pb.axes.add_artist(pasta1)
        # Thinner pasta
        for i in range(0,20):
            y=0.3*float(i)/20.0
            if (y<0.04):
    ...
    def mass_limits(self,ord):
        self.pb.axes.text(0.58,0.225,
                     ...
                     ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.58,0.175,
                     r'$I=50-200~\mathrm{M}_{\odot}~\mathrm{km}^2$',
                     fontsize=self.font_scale*0.7,
                     color=self.text_color,va='center',
                     ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.58,0.125,(r'$\varepsilon_{\mathrm{core}}='+
                                 r'500-1600~\mathrm{MeV}/\mathrm{fm}^{3}$'),
                     fontsize=self.font_scale*0.7,
                     color=self.text_color,va='center',
                     ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.58,0.075,(r'$n_{B,\mathrm{max}}='+
                                 r'0.6-1.3~\mathrm{fm}^{-3}$'),
                     fontsize=self.font_scale*0.7,
                     color=self.text_color,va='center',
                     ha='left',zorder=ord,
                     bbox=dict(facecolor=self.bkgd_color,lw=0))
        self.pb.axes.text(0.58,0.025,
    ...
    """
    def title(self,ord):
        self.pb.axes.text(0.05,0.95,'A neutron star',
