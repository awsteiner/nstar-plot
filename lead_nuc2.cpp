...
int main(void) {
  ...
  fout << "#!/bin/bash" << endl;
  fout << "# This script automatically generated by lead_nuc.cpp."
       << endl;
  ...
  fout << "-td-mat prot 1 0 0 \"alpha=0.9,alpha_mode=mask\" \\" << endl;
  fout << "-td-mat neut 0 0 1 \"alpha=0.9,alpha_mode=mask\" \\" << endl;
  ...
  for(size_t i=0;i<imax;i++) {
    fout << "-td-icos \"(" << tn.get("x",i) << ")\" \"("
         << tn.get("y",i) << ")\" \"("
         << tn.get("z",i) << ")\" \"r=0.8,n_subdiv=2,mat=neut\" \\" << endl;
  }

  // Protons
  for(size_t j=0;j<jmax;j++) {
    fout << "-td-icos \"(" << tp.get("x",j) << ")\" \"("
         << tp.get("y",j) << ")\" \"("
         << tp.get("z",j) << ")\" \"r=0.8,n_subdiv=2,mat=prot\" \\" << endl;
  }

  fout << "-gltf lead_nuc.gltf" << endl;
